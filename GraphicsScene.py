...
class GraphicsScene(QWidget):
    def __init__(self, parent=None):
        ...
        self.trackElement = TrackElement(self)
        self.trackElement.setPosition(QPoint(10, 25))
        self.trackElement.isMovable = False
        self.addElement(self.trackElement)
    ...
    def mousePressEvent(self, e):
        ...
        if self.grabbed_element is not None:
            ...
            self.grabbed_element.mousePress()
            self.selected_elements.clear()
            self.selected_elements.append(self.grabbed_element)
            self.update()
        elif self.selected_elements is not None:
            self.selected_elements.clear()
            self.update()
    ...
    def mouseReleaseEvent(self, e):
        ...
        grabbedElement = self.grabbed_element
        if self.grabbed_element is not None:
            self.grabbed_element.mouseRelease()
            self.grabbed_element = None
            self.moved = False
            update = True

        if grabbedElement is not None and grabbedElement.boundingBox.contains(e.pos()):
            grabbedElement.mouseClick()

        # Scenario:
        # If an element is dragged moved behind an another element and now if mouse is released then as the later
        # element is above the previous element so the top most element's onEnter should be called. The following logic
        # also calls onLeave function of the lower element as it is no more visible.
...
class TrackElement(Element):
    ...
    def paint(self, painter):
        ...
        self.size = QSize(self.parent.width() - 20, 8)
        self.boundingBox = QRect(self.boundingBox.topLeft(), self.size)

        painter.drawRect(QRect(QPoint(), self.boundingBox.size()))
